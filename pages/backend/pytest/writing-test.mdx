...
With `__tracebackhide__ = True`, failing tests will not include assertion helper function in the traceback.

We can also use `assert c1.id == c2.id, "id's don't match."` at line 8, 9 with the same effect.
