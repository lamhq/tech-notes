...
2. Setup your data source:
  ```ts filename="src/data-source.ts"
...
3. Prepare TypeORM entity:
  ```ts filename="src/entity/user.entity.ts"
...
```sh
npm run typeorm migration:create src/migration/AddSampleUsers
```
...
```sh
npm run typeorm migration:generate PostRefactoring -- -d src/data-source.ts
```
